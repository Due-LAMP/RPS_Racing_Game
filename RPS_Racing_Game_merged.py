...
SHOW_GESTURE_SEC = 1.0
SHOW_SEQ_SEC = SHOW_GESTURE_SEC * 3  # ROCK -> SCISSORS -> PAPER
RECOG_SEC = 2.0                      # 마지막 2초 인식 구간
VOTE_WINDOW = RECOG_SEC
...
# Colors — racing palette
P1_COLOR = (255, 140, 70)   # orange-red racer
P1_MID   = (225, 95, 40)
P1_DARK  = (120, 45, 20)
P1_GLOW  = (255, 185, 120)
P2_COLOR = (90, 170, 255)   # blue racer
P2_MID   = (45, 125, 230)
P2_DARK  = (25, 70, 130)
P2_GLOW  = (150, 210, 255)
GOLD        = (70, 220, 255)
NEON_CYAN   = (80, 220, 240)
NEON_PINK   = (95, 120, 255)
BG_DARK     = (28, 30, 35)
BG_PANEL    = (36, 38, 44)
TILE_BORDER = (95, 98, 108)
...
TILE_COLORS = {
    ...
    "mine":        (48, 90, 170),
    "warp":        (165, 95, 35),
    "ladder_down": (65, 125, 160),
}
TILE_GLOW = {
    "rocket":      (125, 170, 255),
    "broken":      (120, 95, 75),
    "ladder_up":   (120, 230, 145),
    "mine":        (95, 160, 245),
    "warp":        (245, 190, 95),
    "ladder_down": (130, 205, 235),
}
TILE_ICON = {
    "rocket": ">>", "broken": "##", "ladder_up": "/\\",
    "mine": "!!",   "warp": "<>",   "ladder_down": "\\/",
}
TILE_LABEL = {
    "rocket": "BOOST", "broken": "OIL", "ladder_up": "RAMP",
    "mine": "SPIKE",   "warp": "SWITCH", "ladder_down": "SLOW",
}
...
def build_track_points(bw, bh):
    cells = build_cells(bw, bh)
    return [cell_center(cells, i) for i in range(min(BOARD_SIZE, len(cells)))]
...
def countdown_stage(elapsed):
    if elapsed < SHOW_GESTURE_SEC:
        return "rock"
    if elapsed < SHOW_GESTURE_SEC * 2:
        return "scissors"
    if elapsed < SHOW_SEQ_SEC:
        return "paper"
    return "recognize"


def lerp_board(track_points, old_p, new_p, t):
    if old_p == new_p:
        return track_points[new_p]
...
def draw_token(panel, px, py, player, pulse=False, trail=False):
    ...
    # glow + shadow
    soft_ring(panel, px, py, r + 1, glow)
    rounded_rect(panel, (px - 14 + 2, py - 8 + 2), (px + 14 + 2, py + 8 + 2), (10, 10, 14), 4)

    # car body
    rounded_rect(panel, (px - 14, py - 8), (px + 14, py + 8), dark, 4)
    rounded_rect(panel, (px - 12, py - 6), (px + 12, py + 6), mid, 3)
    rounded_rect(panel, (px - 8, py - 4), (px + 8, py + 4), color, 2)

    # windshield
    rounded_rect(panel, (px - 5, py - 3), (px + 5, py + 1), (215, 235, 245), 2)

    # wheels
    cv2.circle(panel, (px - 9, py + 8), 3, (18, 18, 22), -1)
    cv2.circle(panel, (px + 9, py + 8), 3, (18, 18, 22), -1)
    cv2.circle(panel, (px - 9, py - 8), 3, (18, 18, 22), -1)
    cv2.circle(panel, (px + 9, py - 8), 3, (18, 18, 22), -1)

    # player marker
    lbl = "1" if player == 0 else "2"
    cv2.putText(panel, lbl, (px - 4, py + 4),
...
def draw_board(bw, bh, positions, old_positions, stuns, phase, elapsed,
    ...
    # ── gradient header bar
    gradient_v(panel, 0, 50, 0, bw, (58, 60, 72), BG_DARK)

    # accent line under header
    cv2.line(panel, (10, 50), (bw - 10, 50), NEON_CYAN, 1, cv2.LINE_AA)

    # title
    put_text_shadow(panel, "RPS CIRCUIT", (12, 24),
    ...
    track_points = build_track_points(bw, bh)
    t_now = time.time()

    # road backdrop area
    gradient_v(panel, 54, bh - 130, 0, bw, (45, 47, 54), (30, 32, 38))

    # main road body
    road_pts = np.array(track_points, dtype=np.int32).reshape(-1, 1, 2)
    cv2.polylines(panel, [road_pts], False, (22, 22, 26), 32, cv2.LINE_AA)
    cv2.polylines(panel, [road_pts], False, (70, 72, 80), 26, cv2.LINE_AA)

    # road edges
    cv2.polylines(panel, [road_pts], False, (120, 122, 132), 2, cv2.LINE_AA)

    # ── ladder / slide shortcuts
    for start, end in LADDER_MAP.items():
        if start >= len(track_points) or end >= len(track_points):
            continue
        sc = track_points[start]
        ec = track_points[end]
        is_up = end > start
    ...
    for i in range(len(track_points) - 1):
        ...
        for s in range(n):
            if s % 2 == 0:
                ...
                cv2.line(panel, a, b, (220, 220, 220), 2, cv2.LINE_AA)

    # index waypoints on road
    for idx, (x, y) in enumerate(track_points):
        cv2.circle(panel, (x, y), 3, (170, 175, 190), -1)
        cv2.putText(panel, str(idx), (x - 6, y - 10),
                    cv2.FONT_HERSHEY_PLAIN, 0.8, (200, 205, 220), 1, cv2.LINE_AA)

    # start/finish gates
    sx, sy = track_points[0]
    fx, fy = track_points[GOAL_POS]
    cv2.circle(panel, (sx, sy), 11, (90, 190, 90), 2)
    cv2.putText(panel, "START", (sx - 20, sy - 16), cv2.FONT_HERSHEY_PLAIN, 0.9, (170, 255, 170), 1)
    pulse_b = 0.6 + 0.4 * math.sin(t_now * 4)
    finish_c = tuple(int(c * pulse_b) for c in (220, 220, 245))
    cv2.circle(panel, (fx, fy), 12, finish_c, 2)
    cv2.putText(panel, "FINISH", (fx - 20, fy - 16), cv2.FONT_HERSHEY_PLAIN, 0.9, finish_c, 1)

    # special point markers
    for idx, effect in SPECIAL_TILES.items():
        if idx >= len(track_points):
            continue
        x, y = track_points[idx]
        col = TILE_GLOW.get(effect, (120, 120, 140))
        pulse = 0.6 + 0.4 * math.sin(t_now * 3 + idx)
        pc = tuple(int(c * pulse) for c in col)
        cv2.circle(panel, (x, y), 8, pc, 2)
        icon = TILE_ICON.get(effect, "")
        text_centered(panel, icon, x, y + 1, cv2.FONT_HERSHEY_SIMPLEX, 0.38, pc, 1)
        cv2.putText(panel, TILE_LABEL.get(effect, ""), (x + 10, y + 4),
                    cv2.FONT_HERSHEY_PLAIN, 0.75, pc, 1, cv2.LINE_AA)

    # ── tokens
    if phase == PHASE_RESOLVE and anim_t > 0:
        for p in range(2):
            pos = lerp_board(track_points, old_positions[p], positions[p], anim_t)
            yoff = -12 if p == 0 else 12
            draw_token(panel, pos[0], pos[1] + yoff, p, trail=True)
    else:
        disp = old_positions if (phase == PHASE_RESOLVE and anim_t == 0) else positions
        for p in range(2):
            c = track_points[disp[p]]
            yoff = -12 if p == 0 else 12
            draw_token(panel, c[0], c[1] + yoff, p, pulse=(phase == PHASE_COUNTDOWN))
    ...
    for p in range(2):
        ...
        cv2.putText(panel, f"CAR {p+1}  Pos:{positions[p]}/{GOAL_POS}{stun_tag}",
    ...
    if phase == PHASE_COUNTDOWN:
        ...
        stage = countdown_stage(elapsed)

        # color stages
        if stage in ("rock", "scissors", "paper"):
            bc = (60, 230, 110)
        else:
            bc = (50, 80, 240)
        ...
        sz = cv2.getTextSize(prompt, cv2.FONT_HERSHEY_SIMPLEX, 1.25, 3)[0]
        tx = (bw - sz[0]) // 2
        ty = iy + 72
        # glow behind text
        glow_c = tuple(int(c * 0.3) for c in bc)
        cv2.putText(panel, prompt, (tx, ty), cv2.FONT_HERSHEY_SIMPLEX, 1.25, glow_c, 6, cv2.LINE_AA)
        cv2.putText(panel, prompt, (tx + 1, ty + 1), cv2.FONT_HERSHEY_SIMPLEX, 1.25, (8, 8, 12), 4, cv2.LINE_AA)
        cv2.putText(panel, prompt, (tx, ty), cv2.FONT_HERSHEY_SIMPLEX, 1.25, bc, 3, cv2.LINE_AA)

        if stage == "recognize":
            recog_remain = max(0.0, ROUND_SEC - elapsed)
            rt = f"Detecting: {recog_remain:.1f}s"
            rsz = cv2.getTextSize(rt, cv2.FONT_HERSHEY_SIMPLEX, 0.45, 1)[0]
            cv2.putText(panel, rt, ((bw - rsz[0]) // 2, iy + 92),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (240, 240, 250), 1, cv2.LINE_AA)

        # blink alert
        if stage == "recognize" and int(t_now * 7) % 2:
            at = "SHOW YOUR HAND!"
...
BOARD_H = 460
cv2.namedWindow("RPS Camera", cv2.WINDOW_NORMAL)
cv2.namedWindow("RPS Board", cv2.WINDOW_NORMAL)
cv2.resizeWindow("RPS Camera", CAM_W, CAM_H)
cv2.resizeWindow("RPS Board", CAM_W, BOARD_H)
...
while cap.isOpened():
    ...
    if phase == PHASE_COUNTDOWN:
        ...
        # accumulate votes only in the last recognition window
        if stage == "recognize":
            if p1_g is not None:
